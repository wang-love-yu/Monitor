...
public class SuitLines extends View {
     ...
     * value：一组数据
     */
    private Map<Integer, List<LineUnit>> datas = new HashMap<>();
    ...
    //y轴默认的值
    private float mYDefaultValue = 100;
    // 曲线、线段
    ...
    private int[] findSuitEdgeInVisual() {
        ...
        if (offset == 0) {// 不可滑动或当前位于最左边
            ...
            if (datas.get(0).size() > maxOfVisible) {
                ...
                List<LineUnit> i = datas.get(0);
                while (low <= high) {
                    int mid = (low + high) >>> 1;
                    LineUnit midVal = i.get(mid);
    ...
    private int[] findSuitEdgeInVisual2() {
        ...
        if (offset == 0) {// 不可滑动或当前位于最左边
            startIndex = 0;
            endIndex = Math.min(datas.get(0).size() - 1, maxOfVisible);
        } else if (Math.abs(offset) == maxOffset) {// 可滑动且当前位于最右边
            endIndex = datas.get(0).size() - 1;
            startIndex = endIndex - maxOfVisible + 1;
        } else {
    ...
    private void drawLines(Canvas canvas, int startIndex, int endIndex) {
        ...
        for (int i = startIndex; i <= endIndex; i++) {
            for (int j = 0; j < datas.size(); j++) {
                ...
                LineUnit current = datas.get(j).get(i);
                if (current == null || current.getXY() == null) {
                    continue;
                }
                if (current.getDays() == LineUnit.TODAY) {
                    if (Integer.parseInt(current.getExtX()) > 14) {
                        continue;
                    }
                }
                ...
                if (lineType == SEGMENT) {
                    ...
                    // 到这里肯定不是起始点，所以可以减1
                    LineUnit previous = datas.get(j).get(i - 1);
    ...
    private void drawClickHint(Canvas canvas) {
        LineUnit cur = datas.get(clickIndexs[1]).get(clickIndexs[0]);
    ...
    private void drawX(Canvas canvas, int startIndex, int endIndex) {
        canvas.drawLine(datas.get(0).get(startIndex).getXY().x, xArea.top,
                // datas.get(0).get(endIndex).getXY().x,
                xArea.right - basePadding,
                xArea.top, xyPaint);
        for (int i = startIndex; i <= endIndex; i++) {
    ...
    private void feedInternal(Map<Integer, List<LineUnit>> entry, List<Paint> entryPaints, boolean needAnim) {
        cancelAllAnims();
     ...
     * @param datas
     */
    private void calcMaxUnit(Map<Integer, List<LineUnit>> datas) {
        // 先“扁平”
        List<LineUnit> allLineUnits = new ArrayList<>();
        for (List<LineUnit> line : datas.values()) {
            allLineUnits.addAll(line);
        }
        // 再拷贝，防止引用问题
        List<LineUnit> bakLineUnits = new ArrayList<>();
        for (int i = 0; i < allLineUnits.size(); i++) {
            bakLineUnits.add(allLineUnits.get(i).clone());
        }
        // 最后排序，得到最大值
        Collections.sort(bakLineUnits);
        LineUnit maxLineUnit = bakLineUnits.get(bakLineUnits.size() - 1);
        LineUnit minLineUnit = bakLineUnits.get(0);
        minAndMaxOfY[0] = Util.getCeil5(Math.min(minLineUnit.getValue(), 0));
        minAndMaxOfY[1] = Util.getCeil5(Math.max(maxLineUnit.getValue(), 0));
        if (minAndMaxOfY[1] == 0) {
            minAndMaxOfY[1] = mYDefaultValue;
        }
    }
    ...
    private void calcUnitXY() {
        ...
        int realNum = Math.min(datas.get(0).size(), maxOfVisible) + 1;
        realBetween = linesArea.width() / (realNum);
        ...
        Log.d(TAG, "calcUnitXY: yDefault= " + zeroAxisValue);
        /*if (zeroAxisValue == Float.NaN) {
            zeroAxisValue = mYDefaultValue;
        }*/
    ...
    private long calcTotalCost() {
        ...
        return oneLineCost + oneLineCost / percentOfStartNextLineAnim * (datas.size() - 1) + LineUnit.DURATION + 16;
    }
    ...
    private void showWithAnims() {
        ...
        for (int i = suitEdge[0]; i <= suitEdge[1]; i++) {
            for (List<LineUnit> item : datas.values()) {
    ...
    private void startLineAnim(final int startIndex, final int endIndex) {
        final List<LineUnit> line = datas.get(curAnimLine);
...
//    }
     ...
     * @param line
     */
    public void feedWithAnim(List<LineUnit> line) {
        if (line == null || line.isEmpty()) return;
        final Map<Integer, List<LineUnit>> entry = new HashMap<>();
        entry.put(0, line);
     ...
     * @param line
     */
    public void feed(List<LineUnit> line) {
        if (line == null || line.isEmpty()) return;
        final Map<Integer, List<LineUnit>> entry = new HashMap<>();
        entry.put(0, line);
        handler.post(new Runnable() {
    ...
    public static class LineBuilder {
        ...
        private Map<Integer, List<LineUnit>> datas;
         ...
         * @return
         */
        public LineBuilder add(List<LineUnit> data, int... color) {
